...
<html>
  <head>
		...
		<script type="text/javascript" language="javascript">  		
   		//删除单个
   		function deleteSericeTypeById(adminIds){
   			if(window.confirm("此类型内容也会删除，确定要删除吗?")){
	 			window.location.href = "deleteSericeType.do?id="+adminIds;
	 		}
   		}
	</script>
					...
					<table width="100%" border="0" cellspacing="0" cellpadding="0">
						<tr>
							<td width="100" height="28" align="center" bgcolor="#CC0000"
								class="white12">
								<a href="querySericeType.do">服务类型列表</a>
							</td>
							<td width="2">
								&nbsp;
							</td>
							<td width="100" align="center" bgcolor="#8594A9" class="white12">
								<a href="addServiceTypeInit.do">添加服务类型</a>
							</td>
							<td>
								&nbsp;
							</td>
							...
							<s:else>
								<s:iterator value="#request.list" var="bean" status="st">
									<tr class="gvItem">
										...
										<td>
											<a href="updateServiceTypeInit.do?id=${bean.id }" target="main"> 编辑</a>
											<s:if
												test="#bean.id!=1&&#bean.id!=2&&#bean.id!=3&&#bean.id!=4&&#bean.id!=5">
												<a id="gvNews_ctl02_lbDelete" style="cursor: pointer;"
													onclick="deleteSericeTypeById(${bean.id })">删除</a>
											</s:if>
										</td>
									</tr>
								</s:iterator>

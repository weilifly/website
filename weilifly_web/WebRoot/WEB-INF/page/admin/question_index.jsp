  ...
  <head>
	    ...
	    $(function(){
   		...
   		function initListInfo(praData) {
		    praData["name"] = $("#questionName").val();
		    praData["status"] = $("#status").val();
	   		$.shovePost("queryQuestionList.do",praData,initCallBack);
   		}
   		
   		//删除单个
   		function deleteQuestionById(adminIds,pageNow){
	 		if(window.confirm("确定要删除吗?")){
	 			window.location.href = "deleteQuestion.do?id="+adminIds+"&pageNow="+pageNow;
	 		}
   		}
   		...
   		//删除多个选中项
   		function deleteQuestions(pageNow){
   			if(!checked("删除")){
   				return;
   			}
			...
			Ids = Ids.substring(0,Ids.lastIndexOf(","));
		 	window.location.href = "deleteQuestion.do?id="+Ids+"&pageNow="+pageNow;
   		}
						...
						<tbody>
							<tr>
								<td class="f66" align="left" width="50%" height="36px">
									问题名称：
									<input id="questionName" name="paramMap.name" />&nbsp;&nbsp;
									是否显示:

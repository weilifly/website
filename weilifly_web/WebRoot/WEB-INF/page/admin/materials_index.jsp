	...
	<head>
		...
		<script type="text/javascript">
			...
			function initListInfo(praData){
				praData["number"] = $("#number").val();
				praData["name"] = $("#name").val();
				praData["status"] = $("#status").val();
				praData["productId"] = $("#productId").val();
				praData["startDate"] = $("#startDate").val();
				praData["endDate"] = $("#endDate").val();
				praData["aId"]=$("#moduleId").val();
		 		$.shovePost("queryMaterialsInfo.do",praData,initCallBack);
		 	}
			...
			function dels(){
		 	...
		 	function checkAll(e){
   			...
   			function delObjs(ids){
   				$.shovePost("deleteMaterials.do",{id:ids},function(data){
   					if(data.msg==1){
   						alert("删除成功！");
					...
					<table width="100%" border="0" cellspacing="0" cellpadding="0">
						<tr>
							...
							<td width="100" align="center" bgcolor="#8594A9" class="white12">
								<a href="addMaterialsInit.do">添加资料</a>
							</td>
							<td width="2">
								&nbsp;
							</td>
							...
							<tbody>
								<tr>
									<td class="f66" align="left" width="50%" height="36px">
										...
										所属产品：
										<s:select name="productId" id="productId" theme="simple"
											list="productList" listKey="id" listValue="name" headerKey="-1" headerValue="-请选择-">
										</s:select>
										&nbsp;&nbsp;
										模块：
										<s:select name="moduleId" id="moduleId" theme="simple"
											list="attributeModuleList" listKey="id" listValue="name" headerKey="-1" headerValue="-请选择-">
										</s:select>	
										&nbsp;&nbsp;
										是否显示：

					...
					<input type="hidden" id="roleId" value="${paramMap.id }" />
						...
						<input class="top rightId" name="rightsId" id="top_${bean.id}" type="checkbox" value="${bean.id}" />${bean.summary }<br/>
							<s:iterator value="rightsList.{?#this.parentId==#bean.id}" var="sBean">
								&nbsp;&nbsp;&nbsp;<input class="top_${bean.id} parent rightId" name="rightsId" id="parent_${sBean.id }" parentId="${sBean.parentId }" type="checkbox" value="${sBean.id}" />${sBean.summary }<s:iterator value="rightsList.{?#this.parentId==#sBean.id}" var="sbBean">&nbsp;&nbsp;&nbsp;<input class="top_${bean.id} bottom parent_${sBean.id} rightId" name="rightsId" parentId="${sbBean.parentId }"  type="checkbox" value="${sbBean.id}" />${sbBean.summary }</s:iterator><br/>
							</s:iterator>
							<br/>
		             </s:iterator>
									</td>

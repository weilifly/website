		...
		<script type="text/javascript">
			...
			KindEditor.ready(function(K) {
				...
				/*$(".content").each(function(i, n){
					var editor_content+i = K.create('textarea[id="content_"'+i+']', {
						cssPath : '../kindeditor/plugins/code/prettify.css',
						uploadJson : 'kindEditorUpload.do',
						fileManagerJson : 'kindEditorManager.do',
						allowFileManager : true,
				...
				/*editor_content = K.create('textarea[class="content"]', {
					cssPath : '../kindeditor/plugins/code/prettify.css',
					uploadJson : 'kindEditorUpload.do',
					fileManagerJson : 'kindEditorManager.do',
					allowFileManager : true,
			...
			$(function(){
				$("#btn_save").click(function(){
					var oEven;
					var colorIdImageStocks = [];
					
					$(".tr_color").each(function(i, n){
						oEven = $(n);
					 	if(!oEven.is(":hidden")){
					 		colorIdImageStocks.push(oEven.attr("color_id")+"ｒ"+oEven.children().find(".content").val());
					 	}
					...
					param["parameters"] = colorIdImageStocks.join("ɑ");
					$.shovePost("addProduct.do",param,function(data){
						alert(data.error);
						if(data.returnId>0){
				...
				$("#bt_color").click(function(){
					...
					$("input[name=in_color]:checked").each(function(){
						colorIds.push($(this).val());
					});
					
					$(".tr_color").each(function(){
						even = $(this);
						color_id = even.attr("color_id");
						
						for(j=0;j<colorIds.length;j++){
							if(color_id == colorIds[j]){
								even.children("td[c_type=seq]").html(i);
								even.show();
								i++;
								break;
							}
					...
					<div style="width: auto; background-color: #FFF; padding: 10px;">
						<table width="100%" border="0" cellspacing="1" cellpadding="3">
							...
							<tr>
								...
								<td align="left" class="f66">
									<s:radio name="paramMap.isShow" id="isShow" theme="simple" value="1" list="#{1:'是',2:'否'}" />
							...
							<tr>
								...
								<td align="left" class="f66">
									<s:iterator value="parameterList" var="bean">
										<input type="checkbox" name="in_color" value="${bean.id }" />${bean.name }&nbsp;&nbsp;
									</s:iterator>
									<input type="button" id="bt_color" value="选择"/>
								</td>
							</tr>
							<tr>
							<tr>
								...
								<td align="left" class="f66">
									<table style="border: 1px solid #CCCCCC;text-align: center;">
										<tr>
											<th >序号</th>
											<th>参数</th>
											<th>内容</th>
										</tr>
										<s:iterator value="parameterList" var="bean" status="sta">
											<tr class="tr_color" color_id="${bean.id }" style="display: none;" >
												<td c_type="seq" >1</td>
												<td c_type="name" >${bean.name }</td>
												<td><textarea class="content" id="content_${sta.index}" rows="10" class="textareash" style="width: 500px; padding:5px;"></textarea></td>
											</tr>
										</s:iterator>
									</table>
								</td>
							...
							<tr>
								...
								<td align="left" class="f66">
									<textarea id="details" rows="20" class="textareash" style="width: 700px; padding:5px;"></textarea>
								</td>
							</tr>
							<tr>
								<td style="height: 25px;" align="right" class="blue12">
